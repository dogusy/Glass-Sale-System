  ...
  <div  class="modal fade" id="myModal" role="dialog">
    <div style="height:300px "  class="modal-dialog">
      ...
      <div  class="modal-content">
        ...
        <form action="woman.php" method="GET" autocomplete="off" >
        <div  class="containerfilter">
            <?php  
                        $databrand=mysqli_query($baglan,"select distinct  Brand  from glass where Gender='Kadın'");
                        $datashape=mysqli_query($baglan,"select Frameshape, Frameshapeicon  from glass  where Gender='Kadın' group by Frameshape");
                        $dataglass=mysqli_query($baglan,"select distinct  Glassmaterial  from glass where Gender='Kadın'");
                        $datamaterial=mysqli_query($baglan,"select distinct  Framematerial  from glass where Gender='Kadın'");
                        $datamodelnumber=mysqli_query($baglan,"select distinct  ModelNumber  from glass where Gender='Kadın'");
                        $dataprice=mysqli_query($baglan,"select distinct  Price  from glass where Gender='Kadın'");
                        $dataframecolor=mysqli_query($baglan,"select distinct  Framecolor , Colorcode  from glass where Gender='Kadın'");
                        $dataproductdiv=mysqli_query($baglan,"select distinct  ProductDiv	  from glass where Gender='Kadın'");
             ?>
...
<?php  
$sorgu="select * from glass where Gender IN ('Kadın') ";
...
//echo '<div style="background-color:blue;" class="container">';
    for($page=1;$page<=$numberofpages;$page++){
        echo '<a  href="woman.php?page='.$page.'">'.$page.'</a>';
    }
 // echo '</div>';
  }
  ?>
