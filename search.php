...
<?php
$sorgu="SELECT * FROM glass where Gender IN ('Erkek','Kadın','Child') ";
$search="";
if(isset($_POST["searchbutton"])&&isset($_POST["search"])){
    $search=mysqli_real_escape_string($baglan,$_POST["search"]);
    $sorgu = "SELECT * FROM glass WHERE   Brand LIKE '%$search%' OR  ModelNumber LIKE '%$search%' OR Framecolor LIKE '%$search%' ";

}
    $main_id=isset($_SESSION['name'])?$_SESSION['name']:10;
    ...
    <div style="height:300px "  class="modal-dialog">
      ...
      <div  class="modal-content">
        ...
        <div  class="containerfilter">
            <?php
                        $databrand=mysqli_query($baglan,"select distinct  Brand  from glass WHERE Brand LIKE '%$search%' OR  ModelNumber LIKE '%$search%' OR Framecolor LIKE '%$search%'");
                        $datashape=mysqli_query($baglan,"select Frameshape, Frameshapeicon  from glass  WHERE Brand LIKE '%$search%' OR  ModelNumber LIKE '%$search%' OR Framecolor LIKE '%$search%' group by Frameshape");
                        $dataglass=mysqli_query($baglan,"select distinct  Glassmaterial  from glass WHERE Brand LIKE '%$search%' OR  ModelNumber LIKE '%$search%' OR Framecolor LIKE '%$search%'");
                        $datamaterial=mysqli_query($baglan,"select distinct  Framematerial  from glass WHERE Brand LIKE '%$search%' OR  ModelNumber LIKE '%$search%' OR Framecolor LIKE '%$search%'");
                        $datamodelnumber=mysqli_query($baglan,"select distinct  ModelNumber  from glass WHERE Brand LIKE '%$search%' OR  ModelNumber LIKE '%$search%' OR Framecolor LIKE '%$search%'");
                        $dataprice=mysqli_query($baglan,"select distinct  Price  from glass WHERE Brand LIKE '%$search%' OR  ModelNumber LIKE '%$search%' OR Framecolor LIKE '%$search%'");
                        $dataframecolor=mysqli_query($baglan,"select distinct  Framecolor , Colorcode  from glass WHERE Brand LIKE '%$search%' OR  ModelNumber LIKE '%$search%' OR Framecolor LIKE '%$search%'");
                        $dataproductdiv=mysqli_query($baglan,"select distinct  ProductDiv	  from glass WHERE Brand LIKE '%$search%' OR  ModelNumber LIKE '%$search%' OR Framecolor LIKE '%$search%'");
             ?>
            ...
            <div id="br" class = "boxfilter" >  
             ...
             <label for="pd">Product Division</label>
...
<h2 style="font-family: 'Montserrat';  text-align: center;  margin: auto;  width: 50%;
">
<?php
    echo "Results not found.";  
  ?></h2> 
  <img style="display: flex;
 margin: auto;  width: 50%;" src="Glasspictures/glassnotfount.jpg" alt="hata"></div>
<?php } ?>
